...
# input dataset
train_jpg = glob.glob('./data/xray_dataset/train/*/*')
...
if __name__ == '__main__':
    ...
    for flod_idx, (train_idx, val_idx) in enumerate(skf.split(train_jpg, train_jpg)):
        print('num_flod:{}'.format(flod_idx))
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            ), batch_size=10, shuffle=True, num_workers=5, pin_memory=True
        )
        ...
        for epoch in range(10):
            ...
            cur_acc = val_acc.avg.item()
            torch.save(model.state_dict(), './mobilenet_pth/mobilenet_fold{0}_epoch{1}.pt'.format(flod_idx,epoch))
            if cur_acc > best_acc:
                best_acc = cur_acc
                torch.save(model.state_dict(), './mobilenet_pth/mobilenet_best.pt')
